...
#   --local    Install into ./.claude/skills/ (current project only)
#   Default:   --global
...
if [ "$SCOPE" = "global" ]; then
  SKILLS_DEST="$HOME/.claude"
  echo -e "\nScope: ${BOLD}global${RESET} — skills available in every project"
  echo -e "Installing into: ${BOLD}$HOME/.claude${RESET}"
else
  SKILLS_DEST="$(pwd)/.claude"
  echo -e "\nScope: ${BOLD}local${RESET} — skills available in this project only"
  echo -e "Installing into: ${BOLD}$(pwd)/.claude${RESET}"
fi

# Config files (CLAUDE.md, AGENTS.md, GEMINI.md) always go in the current project
CONFIG_DEST="."
    ...
    case "$tool" in
      claude)
        for skill in "${SKILLS[@]}"; do
          fetch "$REPO/skills/$skill/SKILL.md" "$SKILLS_DEST/skills/$skill/SKILL.md"
        done
        ok "${#SKILLS[@]} Kaggle skills → $SKILLS_DEST/skills/"
        fetch "$REPO/domains/kaggle/ORCHESTRATION.md" "$SKILLS_DEST/kaggle/ORCHESTRATION.md"
        ok "ORCHESTRATION.md → $SKILLS_DEST/kaggle/"
        ;;
      antigravity)
...
<!-- Full skills available for Claude Code: github.com/OlixIgnacious/agent-skills -->
  ...
  for tool in "${TOOLS[@]}"; do
    case "$tool" in
      claude)
        ...
        ok "${#SDLC_SKILLS[@]} SDLC skills → $SKILLS_DEST/skills/"
        ;;
      antigravity)
        fetch "$REPO/domains/sdlc/AGENTS.md" "$CONFIG_DEST/AGENTS.md"
    ...
    case "$tool" in
      claude)
        fetch "$REPO/skills/research-paper/SKILL.md" \
              "$SKILLS_DEST/skills/research-paper/SKILL.md"
        ok "research-paper skill → $SKILLS_DEST/skills/"
...
install_meta() {
  ...
  if [ "$has_claude" -eq 1 ]; then
    fetch "$REPO/skills/skill-creator/SKILL.md" \
          "$SKILLS_DEST/skills/skill-creator/SKILL.md"
    ok "skill-creator → $SKILLS_DEST/skills/"
  fi
}
